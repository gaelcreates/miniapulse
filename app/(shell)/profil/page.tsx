    ...
    <div className="mx-auto max-w-[1100px]">
      ...
      <section className="rounded-xl border border-line bg-ink-900 p-6">
        <div className="flex items-start gap-5">
          <div className="relative">
            <span className="inline-flex h-20 w-20 items-center justify-center rounded-2xl bg-accent-cyan font-semibold text-[28px] text-black">MC</span>
            <button
              onClick={() => showToast("Upload photo bientôt disponible", "info")}
              className="absolute -bottom-2 -right-2 inline-flex h-7 w-7 items-center justify-center rounded-full border border-line bg-ink-900 text-white/85 hover:bg-ink-800"
            >
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 font-mono text-[11px] tracking-widest text-white/55">TRUSTMEDIA.FR@GMAIL.COM · MEMBRE DEPUIS NOV 2025</div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center rounded-full border border-line bg-white/[0.02] px-2 py-0.5 text-[10px] tracking-widest text-white/65">FR · CET</span>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-accent-success/15 px-2 py-0.5 text-[10px] tracking-widest text-accent-success">
                <span className="h-1.5 w-1.5 rounded-full bg-accent-success" />SYNC YT ACTIF
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-7 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Nom complet" defaultValue="Marc Lefèvre" />
          <Field label="Email" defaultValue="trustmedia.fr@gmail.com" />
          <Field label="Pseudo YouTube" defaultValue="@marclefevre" />
          <Field label="Niche principale" defaultValue="Mindset · Coaching" />
          <Field label="Langue" defaultValue="Français · English" />
          <Field label="Fuseau" defaultValue="Europe/Paris" />
        </div>

        <div className="mt-6 flex items-center justify-end gap-2">
          <button
            onClick={() => showToast("Modifications annulées", "info")}
            className="rounded-md border border-line bg-white/[0.02] px-4 py-2 text-[12px] text-white/70 hover:bg-white/[0.05]"
          >
            Annuler
          </button>
          <button
            onClick={handleSave}
            className={`rounded-md px-4 py-2 text-[12px] font-semibold transition-all ${saved ? "bg-accent-success text-black" : "bg-accent-cyan text-black hover:shadow-[0_8px_28px_-8px_rgba(52,224,255,0.55)]"}`}
          >
      ...
      <section className="mt-6 rounded-xl border border-line bg-ink-900">
        <div className="border-b border-line px-6 py-4">
          ...
          <div className="mt-1 text-[12.5px] text-white/45">Sync auto · CTR remonté toutes les 4h</div>
        </div>
        <div className="flex items-center justify-between gap-4 px-6 py-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="font-mono text-[11px] tracking-widest text-white/55">42.3K ABONNÉS · 89 VIDÉOS</div>
            </div>
          </div>
          <button
            onClick={() => showToast("Gestion YouTube bientôt disponible", "info")}
...
function Field({ label, defaultValue }: { label: string; defaultValue: string }) {
  return (
    <div>
      <div className="font-mono text-[10px] tracking-[0.18em] text-white/45">{label.toUpperCase()}</div>
      <input
