...
export default function PlanPage() {
  return (
    <div className="mx-auto max-w-[1100px]">
      <div className="mb-10">
        <h1 className="font-display text-[64px] leading-[0.92] tracking-tight sm:text-[80px]">PLAN & CRÉDITS</h1>
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-white/60">Ton abonnement, tes crédits du mois, tes factures.</p>
      </div>
      ...
      <section className="overflow-hidden rounded-xl border border-line bg-ink-900">
        <div className="relative p-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="font-mono text-[10px] tracking-[0.18em] text-white/45">PLAN ACTUEL</div>
              <div className="mt-2 flex items-baseline gap-3">
                <h2 className="font-display text-[44px] leading-none tracking-tight">STUDIO</h2>
                <span className="font-mono text-[14px] tracking-widest text-white/55">79€ / MOIS</span>
              </div>
              <p className="mt-2 max-w-md text-[13px] text-white/55">Le rythme d&apos;un solo qui publie 2 fois par semaine.</p>
              <div className="mt-3 flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-accent-cyan/15 px-2 py-0.5 text-[10px] tracking-widest text-accent-cyan">FACTURATION MENSUELLE</span>
                <span className="inline-flex items-center rounded-full border border-line bg-white/[0.02] px-2 py-0.5 text-[10px] tracking-widest text-white/65">RENOUVELLE LE 12 MAI</span>
              </div>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              <button
                onClick={() => showToast("Changement de plan bientôt disponible", "info")}
                className="rounded-md border border-line bg-white/[0.02] px-4 py-2.5 text-[12px] text-white/75 hover:bg-white/[0.05]"
              >
                Changer de plan
              </button>
              <button
                onClick={() => showToast("Upgrade vers Agence — contact@miniapulse.app", "info")}
                className="rounded-md bg-accent-cyan px-4 py-2.5 text-[12px] font-semibold text-black hover:shadow-[0_8px_28px_-8px_rgba(52,224,255,0.55)]"
              >
                Passer à Agence →
              </button>
            </div>
        ...
        <div className="border-t border-line p-6">
          <div className="flex items-baseline justify-between">
            <div className="font-mono text-[10px] tracking-[0.18em] text-white/45">CRÉDITS DU MOIS</div>
            <div className="font-mono text-[11px] tracking-widest text-white/55">RESET LE 12/05</div>
          </div>
          <div className="mt-2 flex items-baseline gap-3">
            <span className="font-display text-[56px] leading-none tracking-tight text-accent-cyan">218</span>
            <span className="font-display text-[20px] leading-none tracking-tight text-white/55">/ 300</span>
            <span className="ml-auto font-mono text-[11px] tracking-widest text-white/55">82 UTILISÉS · 8 JOURS RESTANTS</span>
          </div>
          <div className="mt-4 h-3 w-full overflow-hidden rounded-full bg-white/[0.05]">
            <div className="h-full rounded-full bg-gradient-to-r from-accent-cyan to-accent-success" style={{ width: "73%" }} />
          </div>
        </div>
      </section>
      ...
      <section className="mt-6 rounded-xl border border-line bg-ink-900">
        <div className="border-b border-line px-5 py-4">
          ...
          <div className="mt-1 text-[12.5px] text-white/45">Changement immédiat · proratisé</div>
        </div>
      ...
      <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div className="rounded-xl border border-line bg-ink-900 lg:col-span-5">
          <div className="border-b border-line px-5 py-4 text-[14px] font-semibold">Méthode de paiement</div>
          <div className="flex items-center justify-between gap-4 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="font-mono text-[14px] tracking-widest text-white/85">•••• 4242</div>
                <div className="mt-0.5 font-mono text-[11px] tracking-widest text-white/45">EXP 12/27 · MARC LEFÈVRE</div>
              </div>
            </div>
            <button
              onClick={() => showToast("Modification du paiement bientôt disponible", "info")}
          ...
          <table className="w-full">
            ...
            <tbody>
              {[
                ...
                <tr key={inv.id} className={`text-[13px] ${i % 2 === 0 ? "" : "bg-white/[0.015]"}`}>
...
function PlanCol({ name, price, credits, features, current }: { name: string; price: string; credits: string; features: string[]; current?: boolean }) {
  return (
    <div className={`relative bg-ink-900 p-6 ${current ? "ring-1 ring-inset ring-accent-cyan/40" : ""}`}>
      {current && (
        <div className="absolute right-3 top-3">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-accent-cyan/15 px-2 py-0.5 text-[10px] tracking-widest text-accent-cyan">
            <span className="h-1.5 w-1.5 rounded-full bg-accent-cyan" />ACTUEL
          </span>
        </div>
      )}
      ...
      <div className="mt-2 font-mono text-[11px] tracking-widest text-white/55">{credits.toUpperCase()}</div>
      ...
      <button
        onClick={() => !current && showToast(`Passage au plan ${name} bientôt disponible`, "info")}
        className={`mt-6 w-full rounded-md py-2 text-[12px] transition-colors ${current ? "border border-line bg-white/[0.02] text-white/55 cursor-default" : "bg-white/[0.06] text-white hover:bg-white/[0.1]"}`}
      >
